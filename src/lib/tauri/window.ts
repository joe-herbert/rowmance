...
export function syncTrafficLightPosition(): void {
  const spacingStr = getComputedStyle(document.documentElement).getPropertyValue('--panel-spacing').trim();
  ...
  invoke('window_set_traffic_light_position', { x, y }).catch(() => {/* non-macOS */ });
}
