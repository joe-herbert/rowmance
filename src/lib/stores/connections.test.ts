...
function makeProfile(overrides: Partial<ConnectionProfile> = {}): ConnectionProfile {
  return {
    ...
    dialectInfo: {
      ...
      defaultColumnType: 'SERIAL',
      commonColumnTypes: ['INTEGER', 'TEXT', 'VARCHAR(255)', 'BOOLEAN'],
      ...
      dropSchemaCascade: true,
      isFileBased: false,
      defaultPort: 5432,
      urlSchemes: ['postgres', 'postgresql'],
      urlTemplate: 'postgres://{username}:{password}@{host}:{port}/{database}',
      fkViolation: null,
      editorDialect: 'postgresql',
      explainFormat: 'postgres_json',
