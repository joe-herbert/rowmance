<script lang="ts">
  ...
  interface Props { connectionId: string; canKill: boolean; }
...
<style>
  .locks { display: flex; flex-direction: column; height: 100%; overflow: hidden; }
  ...
  .kill-btn { padding: 2px 8px; border-radius: var(--radius-sm); font-size: 11px; cursor: pointer; background: color-mix(in srgb, var(--color-danger, #e53e3e) 12%, transparent); border: 1px solid color-mix(in srgb, var(--color-danger, #e53e3e) 30%, transparent); color: var(--color-danger, #e53e3e); transition: background var(--transition-fast); }
  .kill-btn:hover { background: color-mix(in srgb, var(--color-danger, #e53e3e) 20%, transparent); }
</style>
