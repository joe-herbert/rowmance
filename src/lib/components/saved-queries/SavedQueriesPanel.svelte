<script lang="ts">
  ...
  import type { FileQuery, FileQueryFolder } from '$lib/types';
        ...
        {:else}
          <button
            ...
            title={query.connectionStatus === 'unresolved'
              ...
              : `Open ${query.name}`}
          >
            <span class="query-icon" aria-hidden="true"><SavedQueryIcon width={13} height={13} /></span>
      ...
      {#each items as tlItem (tlItem.kind === 'query' ? tlItem.item.id : 'f:' + tlItem.item.id)}
        ...
        {:else}
          ...
          >
            ...
            >
              <span class="folder-icon" aria-hidden="true"><FolderIcon width={13} height={13} /></span>
